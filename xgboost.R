...
# xg.model$evaluation_log[50]
#    iter train_auc
# 1:   50  0.989162

auc <- xg.model$evaluation_log[[2]][50]
auc
# [1] 0.989162
...
gain <- xgb.importance(colnames(training_set[,-1]), model = xg.model)

gain
#                                               Feature        Gain       Cover Frequency
# 1:               feature_47.binned _ xmisc. level pos. 0.590317318 0.709015893      0.36
# 2:               feature_20.binned _ xmisc. level pos. 0.146515847 0.133940846      0.14
# 3:               feature_22.binned _ xmisc. level pos. 0.142252163 0.070944130      0.22
# 4:               feature_38.binned _ xmisc. level pos. 0.070428094 0.044004516      0.14
# 5:               feature_45.binned _ xmisc. level pos. 0.044746694 0.033775155      0.11
# 6:                       feature_68.binned _ x(1, Inf] 0.001953043 0.002786202      0.01
# 7:               feature_77.binned _ xmisc. level pos. 0.001898774 0.002781572      0.01
# 8: feature_16.binned _ xmisc. level pos. + MA01 + AS17 0.001888068 0.002751686      0.01

write.csv(gain,"./gain.csv") 

############################# prediction accuracy #################################
prob_test <- predict(xg.model, as.matrix(testing_set[,-1]))
pre <- ifelse(prob_test > 0.5, 1,0)
...
prob_train <- predict(xg.model, as.matrix(training_set[,-1]))

summary(prob_train)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.04045 0.04045 0.04045 0.07900 0.04045 0.73385 

breaks <- seq(from = 0.04045, to = 0.73385, length.out = 11)
ordered_prob <- sort(prob_train, decreasing = T)
grouped_prob <- cut(ordered_prob, breaks = breaks)


table(grouped_prob)
# grouped_prob
# (0.0404,0.11]  (0.11,0.179] (0.179,0.248] (0.248,0.318] (0.318,0.387] 
# 20986           976             8           324           312 
# (0.387,0.456] (0.456,0.526] (0.526,0.595] (0.595,0.665] (0.665,0.734] 
# 535             0           522             0           144



ordered_bad_label_training <- training_set[order(prob_train,decreasing = T),1]
count_of_1 <- tapply(ordered_bad_label_training, grouped_prob, sum)
count_of_1
# (0.0404,0.11]  (0.11,0.179] (0.179,0.248] (0.248,0.318] (0.318,0.387] 
#             3            57             0            56            46 
# (0.387,0.456] (0.456,0.526] (0.526,0.595] (0.595,0.665] (0.665,0.734] 
#           273            NA           354            NA           131
count_of_all_labels <- tapply(ordered_bad_label_training, t, length)

tmp <- count_of_1 / count_of_all_labels
tmp
# (0.0404,0.11]  (0.11,0.179] (0.179,0.248] (0.248,0.318] (0.318,0.387] 
# 0.0001429524  0.0584016393  0.0000000000  0.1728395062  0.1474358974 
# (0.387,0.456] (0.456,0.526] (0.526,0.595] (0.595,0.665] (0.665,0.734] 
# 0.5102803738            NA  0.6781609195            NA  0.9097222222



rank_ordering <- tmp[seq(from = 10, to = 1, length.out = 10)]
rm(tmp)

rank_ordering
# (0.665,0.734] (0.595,0.665] (0.526,0.595] (0.456,0.526] (0.387,0.456] 
# 0.9097222222            NA  0.6781609195            NA  0.5102803738 
# (0.318,0.387] (0.248,0.318] (0.179,0.248]  (0.11,0.179] (0.0404,0.11] 
# 0.1474358974  0.1728395062  0.0000000000  0.0584016393  0.0001429524 

write.csv(rank_ordering,"./rank_ordering.csv") 
